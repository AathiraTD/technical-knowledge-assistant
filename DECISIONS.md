...
Final counts: 94 published documents plus one staff-tagged evaluation fixture, 552 passages, longest 3,674 characters against a 4,000 ceiling, 49 of the 94 classed clean and 45 partial, none failed. The active versions carry 93 tagged caveat sentences, and the table holds 93 — the current index was built clean, so no superseded caveats sit behind the active ones. An earlier build recorded 99 active against 119 stored, the extra 20 belonging to superseded versions that are retained but never retrieved; that gap is what the retention behaviour looks like once a corpus has changed under it, and it reappears on the next supersession rather than having been designed away. The passage count fell from 644 once the download furniture on product pages was removed — see the note under decision 8.
...
**Capability criteria**, weighted by what each failure costs. Note the consequence peculiar to this architecture: the eight checks are deterministic and run before printing, so a weaker model does not produce wrong answers here — it produces refusals. Capability shows up as **coverage**, and the weights follow the checks.
...
retrieval, the policy gate, the eight post-generation checks, the calculations and
...
- **Qualitative synthesis is the weakest point.** The eight checks bound numbers, names, attribution, product scope, product relationships and the asked-for term; they reduce, not eliminate, an invented "this is fine on cob".
