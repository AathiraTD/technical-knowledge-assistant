...
| Component | What it does | Why it exists | Shortcoming |
|---|---|---|---|
| **Chunk store** (built) | Per chunk: embedding, product, document, section, date, authority rank, audience tag. Manifest: every document with type, authority rank, audience tag, date, link and its caveat sentences; excluded documents by name and link; contact text and the product, colour and merchant name lists from the crawl; embedding-model tag and chunking version — the engine refuses to run on a mismatch of either | Retrieval, citation by document name, newest-wins, the staff/public filter, document requests and the appended caveats all read from here | Prototype is an array in a file: one process, one user |
...
| **CLI** (built) | Question and mode in; answer, sources, refusal, diagnostics out | The brief's required interface, in the brief's format | Single user, single turn; real enquiries run six turns |
